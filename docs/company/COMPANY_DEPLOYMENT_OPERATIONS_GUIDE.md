...
## 19. Company 动态首页与发行版本

Company v1.0.0 起提供独立的公司首页：保留登录、个人控制台、管理员控制台、语言和主题切换；不展示供应商品牌墙、价格宣传或演示终端。首页展示的隐藏不改变账号、模型、计费 API 或出口策略。动画可暂停，并尊重系统“减少动态效果”偏好；图形不代表实时安全状态。

首页选择顺序仍为：管理员自定义首页内容 → “简洁首页” → Company 动态首页 → 官方默认首页。采用本次动态首页时，在系统设置中将“自定义首页内容”留空、关闭“简洁首页”。已有站点名称、Logo、副标题和文档链接不会被部署覆盖；要与设计稿一致，可将站点名称改为 `HS AI 工作台`，副标题按公司需求填写。未设置文档链接时，“接入指南”显示内置三步说明。

版本分为三个独立信息：

| 信息 | 来源 | 用途 |
| --- | --- | --- |
| `Company v1.0.0` | 仓库根目录 `COMPANY_VERSION` | 日常可读发行编号 |
| 官方基线，如 `v0.2.0` | `backend/cmd/server/VERSION` | 说明该构建采用的官方版本 |
| `company-完整40位SHA` | 后端原始版本信息 | 精确追溯构建、排查和回滚 |

页面、侧栏和版本详情使用同一个 Company 显示组件。点击版本按钮可查看官方基线和完整识别码，并复制版本信息（兼容内部 HTTP）。语义版本只属于前端展示元数据，不替换后端 `Version/Commit/BuildType`，不改变 SHA 校验，也不开放官方在线更新或在线回退。只有完整 `company-40位小写十六进制SHA` 格式使用新展示；旧式 Company 格式仍由后端 `build_type=company` 保持更新保护。

维护者发布新的 Company 功能时，在审核的同一提交更新 `COMPANY_VERSION`：修复通常增加 patch（如 `1.0.1`），向后兼容功能增加 minor（如 `1.1.0`），明确批准的破坏性变化才增加 major。此文件不会因为刷新页面、每次构建或单纯拉取上游而自动递增。两套系统分支共享发行编号，但各自有不同构建 SHA；前端版本元数据随对应 CI 制品嵌入 binary，不能把新前端单独混配到旧后端来判断服务版本。

**不要为公司发行创建 `v1.0.0` 这种标签**：仓库继承的官方 release workflow 匹配 `v*`，与 Company 安全构建不是同一发布链路。如确需记录公司标签，应使用 `company-v1.0.0`；公司制品仍由 Company 分支的 CI 构建，标签本身不代表制品已完成。部署继续使用原有经过检查的公司脚本，不需要改账号和数据库配置。
